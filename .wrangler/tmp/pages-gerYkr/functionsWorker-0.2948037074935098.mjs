...
async function onRequestGet2({ request, env }) {
  const expected = env.ADMIN_TOKEN;
  if (!expected) {
    return json2({ error: "ADMIN_TOKEN is not set on this project" }, 503);
  }
  const provided = request.headers.get("x-admin-token") || "";
  ...
  if (diff !== 0) return json2({ error: "unauthorized" }, 401);
  const media = env.MEDIA;
  if (!media) {
    return json2({ error: "No MEDIA KV namespace is bound to this project" }, 503);
  }
  const files = [];
  let cursor;
  do {
  ...
  return json2({ ok: true, count: files.length, files });
}
...
function safeName(name) {
  const base = String(name || "file").split(/[\\/]/).pop().replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(-90);
  return base || "file";
}
...
async function onRequestPost2({ request, env }) {
  const expected = env.ADMIN_TOKEN;
  if (!expected) {
    return json3({ error: "ADMIN_TOKEN is not set on this project" }, 503);
  }
  const provided = request.headers.get("x-admin-token") || "";
  ...
  if (!file || typeof file === "string") {
    return json3({ error: "no file field in the form" }, 400);
  }
  if (file.size === 0) return json3({ error: "file is empty" }, 400);
  ...
  const key = `${PREFIX}${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${name}`;
  await media.put(key, await file.arrayBuffer(), {
    metadata: { name, type, size: file.size, uploaded: (/* @__PURE__ */ new Date()).toISOString() }
  });
  return json3({
    ok: true,
    key,
...
// ../.wrangler/tmp/pages-gerYkr/functionsRoutes-0.7871145454359819.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-Tp7xUy/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-Tp7xUy/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2948037074935098.mjs.map
